...
// 1. Create RiskScoreChart.jsx
const chartCode = `import React, { useContext } from 'react';
...
export default function RiskScoreChart() {
  ...
  const sortedZones = [...liveZones].sort((a, b) => b.riskScore - a.riskScore);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-slate-200 p-6 transition-shadow hover:shadow-lg h-full flex flex-col">
      <h3 className="text-base font-semibold text-slate-800 mb-6">Risk Scores by Zone</h3>
...
fs.writeFileSync('src/components/operations/RiskScoreChart.jsx', chartCode);

// 2. Remove chart from DistrictRiskTable.jsx
let tableContent = fs.readFileSync('src/components/operations/DistrictRiskTable.jsx', 'utf8');
tableContent = tableContent.replace(/\{\/\* Chart Section \*\/\}[\s\S]*<\/FadeIn>/, '');
tableContent = tableContent.replace(/<div className="space-y-6">([\s\S]*)<\/div>/, '$1'); // Remove wrapper if needed, or leave it. Actually wait, it's inside <FadeIn direction="up"> <div className="space-y-6"> ... </div> </FadeIn>. It's safer just to remove the chart section.
